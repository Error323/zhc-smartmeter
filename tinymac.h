...
/*! Maximum number of nodes that this node can be aware of (this is the maximum
 * size of the
...
/*! Time for an unregistered node to wait between beacon request transmissions
 * (seconds) */
...
/*! Coordinator grace period to allow after heartbeat expiry before assuming a
 * client has gone (seconds) */
#define TINYMAC_HEARTBEAT_GRACE 2
/*! Time a sleeping node should listen after transmitting or receiving a packet
 * with
...
typedef struct
{
  ...
  tinymac_send_cb_t send_cb;         /*< Callback invoked when pending packet
                                        sent/expires */
...
typedef struct
{
  ...
  uint8_t beacon_interval; /*< 250 ms * 2^n, or
                              TINYMAC_BEACON_INTERVAL_NO_BEACON */
  uint8_t beacon_offset;
} tinymac_params_t;

typedef void (*tinymac_recv_cb_t)(const tinymac_node_t *node, uint8_t type,
...
/*!
 * Send a data packet.
 * NOTE: If used under an OS this function must be called from the same thread
 *that
 ...
 * \param validity	Validity period (in seconds) for packets sent to a sleeping
 *node
 * \param cb		Callback invoked on successful delivery or expiry of validity
 *period
