 ...
 * phy-si443x.c
 *
 * PHY driver for SiLabs Si443x transceivers (RFM22B, RFM23B, etc.)
 *
 */
...
#define CFG_FREQUENCY_F0 (868 * MHZ + 48000)
/*! Channel spacing (kHz) */
...
static const uint8_t phy_si443x_config[] TABLE = {
  /* General setup */
  R_GPIO0_CFG, 3, GPIO_RX_STATE, GPIO_TX_STATE,
  0, /* T/R switching for RFM22B */

  R_DATA_ACCESS_CTRL, 1,
  ENPACRX | ENPACTX | ENCRC |
      CRC_CCITT, /* R_DATA_ACCESS_CTRL - enable packet engine, CRC CCITT */

  R_TX_FIFO_CTRL1,
  3,
  TX_WAIT_THRESH &TXAFTHR_MASK, /* R_TX_FIFO_CTRL1 - TX almost full threshold */
  TX_RESUME_THRESH &TXFAETHR_MASK, /* R_TX_FIFO_CTRL2 - TX almost empty
                                      threshold */
  (RX_BLOCK_SIZE - 1) &
      RXAFTHR_MASK, /* R_RX_FIFO_CTRL - RX almost full threshold */

  R_HEADER_CTRL1,
  8, BCEN(0) | HDCH(0), /* R_HEADER_CTRL1 - no header/broadcast check */
  HDLEN(0) |
      SYNCLEN(CFG_SYNC_WORD_LEN), /* R_HEADER_CTRL2 - sync length, no header */
  ...
  (uint8_t)((CFG_SYNC_WORD) >> 8), (uint8_t)((CFG_SYNC_WORD) >> 0),
  ...
  R_CHANNEL, 2, DEF_CHANNEL, /* R_CHANNEL - default, configurable */
  (CFG_CHANNEL_STEP) / 10,   /* R_STEP */

  /* Receiver setup according to spreadsheet */
  R_IF_BANDWIDTH, 10, 0x05, 0x40, 0x0a, 0x03, 0x50, 0x01, 0x99, 0x9a, 0x06,
  0x68, R_AFC_LIMITER, 5, 0x28, 0x00, 0x28, 0x19, 0x29,
...
typedef enum
{
  ...
  /* Receive states */
  stateRx, /*< Rx synced.  To \see stateRxReady, stateRxValid, stateRxInvalid,
              stateRxFull */
  stateRxReady,   /*< Rx FIFO ready for read.  To \see stateRx, stateRxValid,
                     stateRxInvalid, stateRxFull */
  ...
  /* Transmit states */
  stateTx,     /*< Tx FIFO filling, FIFO not full.  To \see stateTxWait,
                  stateStandby, stateListen */
...
int phy_resume(void)
{
  ...
  INFO("Done\n");
  DELAY_MS(1); /* FIXME: What are we really waiting for here - presence of the
                  INFO above seems enough */
  ...
  si443x_write16(R_INT_ENABLE, ENTXFFAFULL | ENTXFFAEM | ENRXFFAFULL |
                                   ENPKSENT | ENSWDET | ENPKVALID | ENCRCERROR |
                                   ENWUT);
  return 0;
}
...
void phy_register_recv_cb(phy_recv_cb_t cb) { phy_si443x_recv_cb = cb; }

void phy_event_handler(void)
{
  static uint8_t payload[MAX_PACKET];
  unsigned int rxsize;
  ...
  case stateRxValid:
    ...
    /* Back to idle state otherwise if the callback tries to send something it
     * won't work.
     * MAC will turn the receiver off at the correct time if we are a sleepy
     * node */
    phy_listen();
    ...
    {
      phy_si443x_recv_cb((const char *)payload, (size_t)rxsize,
                         phy_si443x_rssi);
    }
