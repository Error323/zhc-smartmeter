...
static const char *tinymac_node_states[] = {
  "Unregistered", "Registered", "SendPending", "WaitAck",
};
#endif
...
typedef struct
{
  ...
  boolean_t permit_attach;   /*< Whether or not we are acceptng registration
                                requests */
...
static inline void tinymac_set_timer(tinymac_timer_t *timer,
                                     tinymac_timer_cb_t cb, void *arg,
                                     uint32_t delay)
{
  timer->callback = cb;
  timer->arg = arg;
...
{
  if (timer->callback &&
      ((int32_t)(tinymac_ctx->tick_count - timer->expiry) >= 0))
  {
  ...
  if (node->retries--)
  {
    ...
    else
    {
      ...
      TRACE("OUT (retry): %04X %02X %02X %02X %02X (%zu)\n",
            node->pending_header.flags, node->pending_header.net_id,
            node->pending_header.dest_addr, node->pending_header.src_addr,
            node->pending_header.seq, node->pending_size);
      tinymac_set_timer(&node->ack_timer, tinymac_ack_timeout, node,
                        TINYMAC_MILLIS(TINYMAC_ACK_TIMEOUT));
...
static int tinymac_tx_packet(tinymac_node_t *dest, uint8_t flags_type,
                             const char *buf, size_t size, uint16_t validity,
                             tinymac_send_cb_t cb)
{
  tinymac_header_t hdr;
  phy_buf_t bufs[] = {
    { (char *)&hdr, sizeof(hdr) }, { (char *)buf, size },
  };
  ...
  if (dest)
  {
    ...
    if (dest->state != tinymacNodeState_Registered)
    {
      /* Destination is busy */
      ERROR("Node %02X is %s\n", dest->addr,
            dest->state == tinymacNodeState_Unregistered ? "not registered"
                                                         : "busy");
      return -1;
    }

    if ((flags_type & TINYMAC_FLAGS_ACK_REQUEST) ||
        (dest->flags & TINYMAC_ATTACH_FLAGS_SLEEPY))
    {
    ...
    /* If destination node is sleepy then defer the transmission (packet
     * contents are
     * copied above) */
    if (dest->flags & TINYMAC_ATTACH_FLAGS_SLEEPY)
    {
      ...
      tinymac_set_timer(&dest->validity_timer, tinymac_validity_timeout, dest,
                        TINYMAC_SECONDS(validity));
      dest->state = tinymacNodeState_SendPending;
      return 0;
    }

    /* We are going to send this packet immediately - if an ack is requested
     * then
     * start the timer now */
    if (flags_type & TINYMAC_FLAGS_ACK_REQUEST)
    {
      ...
      tinymac_set_timer(&dest->ack_timer, tinymac_ack_timeout, dest,
                        TINYMAC_MILLIS(TINYMAC_ACK_TIMEOUT));
  ...
  TRACE("OUT: %04X %02X %02X %02X %02X (%zu)\n", hdr.flags, hdr.net_id,
        hdr.dest_addr, hdr.src_addr, hdr.seq, size);
...
static int tinymac_tx_pending(tinymac_node_t *node)
{
  ...
  if (node->state == tinymacNodeState_SendPending)
  {
    ...
    TRACE("PENDING OUT: %04X %02X %02X %02X %02X (%zu)\n", hdr.flags,
          hdr.net_id, hdr.dest_addr, hdr.src_addr, hdr.seq, node->pending_size);
    tinymac_phy_send(bufs, ARRAY_SIZE(bufs), 0);
    if (!(hdr.flags & TINYMAC_FLAGS_ACK_REQUEST))
    {
...
static int tinymac_tx_ack(tinymac_node_t *node, uint8_t seq)
{
  ...
  TRACE("ACK: %04X %02X %02X %02X %02X\n", hdr.flags, hdr.net_id, hdr.dest_addr,
...
static int tinymac_tx_beacon(boolean_t periodic)
{
  ...
  TRACE("BEACON: %04X %02X %02X %02X %02X\n", hdr.flags, hdr.net_id,
        hdr.dest_addr, hdr.src_addr, hdr.seq);
  return tinymac_phy_send(bufs, ARRAY_SIZE(bufs),
                          periodic ? PHY_FLAG_IMMEDIATE : 0);
}
#endif
...
static void tinymac_rx_beacon(tinymac_header_t *hdr, size_t size)
{
  ...
  TRACE("BEACON from %016" PRIX64 " %s\n", beacon->uuid,
        (beacon->flags & TINYMAC_BEACON_FLAGS_SYNC) ? "(SYNC)" : "(ADV)");
#else
  /* For platforms that don't support printing uint64s we just show the lower
   * 32-bit word */
  ...
  case tinymacClientState_BeaconRequest:
    if (beacon->flags & TINYMAC_BEACON_FLAGS_PERMIT_ATTACH)
    {
      ...
      tinymac_tx_packet(&tinymac_ctx->coord,
                        (uint16_t)tinymacType_RegistrationRequest,
                        (const char *)&attach, sizeof(attach), 0, NULL);

      /* Start callback timer */
      tinymac_set_timer(&tinymac_ctx->timer, tinymac_request_timeout, NULL,
                        TINYMAC_MILLIS(TINYMAC_REGISTRATION_TIMEOUT));
    }
    break;
  case tinymacClientState_Registered:
  {
    ...
    for (n = 0; n < size - sizeof(tinymac_header_t) - sizeof(tinymac_beacon_t);
         n++)
    {
      if (beacon->address_list[n] == tinymac_ctx->addr)
      {
        INFO("Polling coordinator for pending data\n");
        tinymac_tx_packet(&tinymac_ctx->coord, (uint16_t)tinymacType_Poll, NULL,
                          0, 0, NULL);
        break;
      }
    }
...
static void tinymac_rx_registration_response(tinymac_header_t *hdr, size_t size)
{
  tinymac_registration_response_t *addr =
      (tinymac_registration_response_t *)hdr->payload;
  ...
  if (addr->uuid != tinymac_ctx->params.uuid)
  {
    ...
    {
      /* This is a registration broadcast intended for another device - just
       * ignore */
      return;
    }
    else
    {
      /* This was unicast to us - we have an address clash or we are being
       * deregistered */
...
{
  tinymac_registration_request_t *attach =
      (tinymac_registration_request_t *)hdr->payload;
...
static void tinymac_rx_deregistration_request(tinymac_header_t *hdr,
                                              size_t size)
{
  tinymac_deregistration_request_t *detach =
      (tinymac_deregistration_request_t *)hdr->payload;
  tinymac_registration_response_t resp;
  tinymac_node_t *node;
...
static void tinymac_recv_cb(const char *buf, size_t size, int rssi)
{
  ...
  if (!((hdr->net_id == tinymac_ctx->net_id &&
         (hdr->dest_addr == tinymac_ctx->addr ||
          hdr->dest_addr == TINYMAC_ADDR_BROADCAST)) ||
        (hdr->net_id == TINYMAC_NETWORK_ANY &&
         hdr->dest_addr == TINYMAC_ADDR_BROADCAST) ||
        (tinymac_ctx->net_id == TINYMAC_NETWORK_ANY &&
         hdr->dest_addr == TINYMAC_ADDR_BROADCAST)))
  {
   ...
   * "permit attach" state */
  if (hdr->net_id == TINYMAC_NETWORK_ANY ||
      hdr->src_addr == TINYMAC_ADDR_UNASSIGNED)
  {
#if WITH_TINYMAC_COORDINATOR
    ...
    else
    {
      ...
      if (tinymac_ctx->params.coordinator)
      {
        ...
        tinymac_node_t dummy = {
          .addr = hdr->src_addr, .state = tinymacNodeState_Registered,
        };
        ...
        ERROR("Forcing deregistration for unregistered address %02X\n",
              hdr->src_addr);
        resp.uuid =
            0; /* Forces all nodes using this short address to re-register */
        resp.addr = TINYMAC_ADDR_UNASSIGNED;
  ...
  if (type >= tinymacType_RawData)
  {
    ...
    {
      tinymac_ctx->rx_cb((const tinymac_node_t *)node,
                         type & TINYMAC_FLAGS_TYPE_MASK, hdr->payload,
                         size - sizeof(tinymac_header_t));
    }
  }
  else
  {
    ...
    case tinymacType_RegistrationRequest:
      if (size <
          sizeof(tinymac_header_t) + sizeof(tinymac_registration_request_t))
      {
        ERROR("Discarding short packet\n");
        return;
      }
    ...
    case tinymacType_DeregistrationRequest:
      if (size <
          sizeof(tinymac_header_t) + sizeof(tinymac_deregistration_request_t))
      {
        ERROR("Discarding short packet\n");
        return;
      }
    ...
    case tinymacType_RegistrationResponse:
      ...
      if (size <
          sizeof(tinymac_header_t) + sizeof(tinymac_registration_response_t))
      {
        ERROR("Discarding short packet\n");
        return;
      }
...
void tinymac_register_recv_cb(tinymac_recv_cb_t cb) { tinymac_ctx->rx_cb = cb; }
...
void tinymac_tick_handler(void *arg)
{
  ...
  if (tinymac_ctx->params.coordinator)
  {
    ...
    /* This is called once per beacon slot (250 ms) - check if a beacon is due
     * in this
     * slot and increment the counter */
    if (((++tinymac_ctx->slot) & ((1 << tinymac_ctx->params.beacon_interval) -
                                  1)) == tinymac_ctx->params.beacon_offset)
    {
      /* Beacon due */
    ...
    for (n = 0; n < TINYMAC_MAX_NODES; n++, node++)
    {
      if (node->state == tinymacNodeState_Registered)
      {
        ...
        if ((int32_t)(node->last_heard +
                      TINYMAC_SECONDS(heartbeat + TINYMAC_HEARTBEAT_GRACE) -
                      now) <= 0)
        {
          /* Node has gone away */
          /* NOTE: We could ping if this isn't a sleeping node but there doesn't
           * seem
...
#endif
  {
    ...
    if (tinymac_ctx->state == tinymacClientState_Unregistered)
    {
      ...
      tinymac_tx_packet(NULL, (uint16_t)tinymacType_BeaconRequest, NULL, 0, 0,
                        NULL);

      /* Set a timer for beacon request timeout */
      tinymac_set_timer(&tinymac_ctx->timer, tinymac_request_timeout, NULL,
                        TINYMAC_SECONDS(TINYMAC_BEACON_REQUEST_TIMEOUT));
    }
...
int tinymac_send(uint8_t dest, uint8_t type, const char *buf, size_t size,
...
void tinymac_register_reg_cb(tinymac_reg_cb_t cb) { tinymac_ctx->reg_cb = cb; }
...
void tinymac_dump_nodes(void)
{
  ...
  printf("Addr  UUID              State             RSSI  Last Heard Ago  "
         "Heartbeat  Sleepy\n");
  printf("---------------------------------------------------------------------"
         "------------\n");
  for (n = 0; n < TINYMAC_MAX_NODES; n++, node++)
  {
    if (node->uuid)
    {
      printf("%02X    %016" PRIX64 "  %16s  %4d  %14u  %9u  %s\n", node->addr,
             node->uuid, tinymac_node_states[node->state], node->rssi,
             (tinymac_ctx->tick_count - node->last_heard) * TINYMAC_TICK_MS /
                 1000,
