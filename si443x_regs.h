 ...
 * si443x_regs.h
 *
 * Register definitions for Si443x based radios
 *
 */
...
/*! Calculate value to load into register 0x09 given desired load capacitance in
 * fF.
 * XTALSHFT is assumed to be disabled. */
#define XLC(cint) ((((cint)-XLC_MIN) / XLC_STEP) & XLC_MASK)
/*! Calculate current crystal load capacitance in fF given value of register
 * 0x09 */
#define CINT(xlc)                                                              \
  ((((xlc)&XLC_MASK) * XLC_STEP) + XLC_MIN + ((xlc) / XTALSHFT) * 3700)
...
/*! Calculate values to load into registers 0x6e, 0x6f given data rate in bps up
 * to 30 kbps */
#define TXDR_LOW(bps) ((((unsigned long long)bps) << 21) / 1000000ULL)
/*! Calculate values to load into registers 0x6e, 0x6f given data rate in bps
 * over 30 kbps */
...
/*! Carrier frequency trim in steps of 156.25 Hz (low band) or 312.5 Hz (high
 * band).
 *  Desired frequency specified in signed Hz.  Note this register is
 * little-endian,
...
/*! Calculate values to load into registers 0x76, 0x77 given channel 0 frequency
 * in Hz */
#define FC(ch0_hz)                                                             \
  (((ch0_hz) - (BAND_OFFSET + BAND_STEP * _BAND(ch0_hz))) * 64UL /             \
   (BAND_STEP / KHZ))
